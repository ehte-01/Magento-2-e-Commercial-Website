...
class Subscriptions
{
    ...
    public function invalidateSubscription($subscription, $params)
    {
        ...
        if (!empty($subscription->latest_invoice))
        {
            ...
            {
                $expectedValues['latest_invoice']['discounts'][0]['coupon']['id'] = $params['discounts'][0]['coupon'];
            }
            else
            {
    ...
    public function setSubscriptionUpdateDetails(\Stripe\Subscription $subscription, $productIds)
    {
        ...
        {
            $date = $stripeSubscriptionModel->getCurrentPeriodStart();
            $day = date("j", $date);
            $sup = date("S", $date);
            $month = date("F", $date);
            $year = date("y", $date);
        ...
        // Next billing date
        $periodEnd = $stripeSubscriptionModel->getCurrentPeriodEnd();
